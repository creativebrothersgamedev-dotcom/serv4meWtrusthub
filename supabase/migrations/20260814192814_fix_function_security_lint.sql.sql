/*
# Harden function privileges and search_path

## Plain-English summary
Tightens the database linter findings from the previous migration:
- Pins a fixed `search_path` on `search_providers` so it cannot be tricked
  by a session that has changed its search path.
- Explicitly removes the ability for signed-out (anon) visitors to call the
  internal `is_admin`, `provider_review_rating`, and `admin_resolve_rating`
  functions - only signed-in users should be able to call them at all, and
  each function still independently checks the caller's identity.

## Security
No behavior changes for legitimate users; this only removes execute
privileges that were unintentionally left open to the `anon` role.
*/
...
REVOKE ALL ON FUNCTION provider_review_rating(uuid, text) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION provider_review_rating(uuid, text) TO authenticated;

REVOKE ALL ON FUNCTION admin_resolve_rating(uuid, text, text) FROM public, anon, authenticated;
